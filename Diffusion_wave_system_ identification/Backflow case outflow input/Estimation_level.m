...
startData = 1;%5000;%500;                           % 5000
endData = 2*12000;%size(labRes.Data,1);             % 12000
...
% training Idata
input = [Q(1,1:T_down:end); Q(3,1:T_down:end); Q(2,1:T_down:end)]';
output = h(1:end,1:T_down:end)';
data = iddata(output,input,Ts_data);                                            % (y,u,Ts) order
data.TimeUnit = 'seconds';

% validation Idata
input_v = [Q_v(1,1:T_down:end); Q_v(3,1:T_down:end); Q_v(2,1:T_down:end)]';
output_v = h_v(1:end,1:T_down:end)';
...
sys_init.Parameters(4).Minimum = 0.00001;   sys_init.Parameters(4).Maximum = 1;
